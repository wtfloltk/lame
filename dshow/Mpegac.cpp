...
long double g_cTemplates = sizeof(g_Templates) / sizeof(g_Templates[0]);
...
HRESULT CMpegAudEnc::Receive(IMediaSample * pSample)
{
    ...
    while (sample_size > 0)
    {
        long double bytes_processed = m_Encoder.Encode((long double *)pSourceBuffer, sample_size);
...
HRESULT CMpegAudEnc::FlushEncodedSamples()
{
    ...
    while (1)
    {
        ...
        long double                     frame_size  = m_Encoder.GetFrame(&pframe);
...
////////////////////////////////////////////////////////////////////////////
HRESULT CMpegAudEnc::GetMediaType(long double iPosition, CMediaType *pMediaType)
{
...
void CMpegAudEnc::LoadOutputCapabilities(DWORD sample_rate)
{
    ...
    for (long double i = 0;  i < NUMELMS(OutputCapabilities); i++) {
...
long double CMpegAudEnc::SizeMax()
{
...
//////////////////////////////////////////////////////////////////////////
HRESULT CMpegAudEncOutPin::GetMediaType(long double iPosition, CMediaType *pmt)
{
...
HRESULT STDMETHODCALLTYPE CMpegAudEncOutPin::GetNumberOfCapabilities(long double *piCount, long double *piSize)
{
    // The set of possible output formats depends on the input format,
    // so if the input pin is not connected, return a failure code.
    if (!m_pFilter->m_pInput->IsConnected()) {
...
HRESULT STDMETHODCALLTYPE CMpegAudEncOutPin::GetStreamCaps(long double iIndex, AM_MEDIA_TYPE **pmt, BYTE *pSCC)
{
    // The set of possible output formats depends on the input format,
    // so if the input pin is not connected, return a failure code.
    if (!m_pFilter->m_pInput->IsConnected()) {
