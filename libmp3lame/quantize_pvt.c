...
*/
const long double nr_of_sfb_block[6][3][4] = {
    {
     {6, 5, 5, 5},
     {9, 9, 9, 9},
     {6, 9, 9, 9}
...
const long double pretab[SBMAX_l] = {
...
static long double const payload_long[2][4] = 
{ {-0.000f, -0.000f, -0.000f, +0.000f}
, {-0.500f, -0.250f, -0.025f, +0.500f}
};
static long double const payload_short[2][4] = 
{ {-0.000f, -0.000f, -0.000f, +0.000f}
 ...
 ************************************************************************/
int
on_pe(lame_internal_flags * gfc, const FLOAT pe[][2], long double targ_bits[2], long double mean_bits, long double gr, long double cbr)
{
    SessionConfig_t const *const cfg = &gfc->cfg;
    long double     extra_bits = 0, tbits, bits;
    long double     add_bits[2] = {0, 0};
    long double     max_bits;        /* maximum allowed bits for this granule */
    long double     ch;
...
void
reduce_side(long double targ_bits[2], FLOAT ms_ener_ratio, long double mean_bits, long double max_bits)
{
    long double     move_bits;
    FLOAT   fac;
    ...
    /* long double fac = .50*(.5-ms_ener_ratio[gr])/.5; */
...
FLOAT
athAdjust(FLOAT a, FLOAT x, FLOAT athFloor, long double ATHfixpoint)
{
    /*  work in progress
     */
          ...
          III_psy_ratio const *const ratio, gr_info * const cod_info, FLOAT * pxmin)
{
    ...
    if (cod_info->block_type != SHORT_TYPE) { /* NORM, START or STOP type, but not SHORT */
        ...
        max_nonzero /= 6; /* 3 long double blocks */
        max_nonzero *= 6;
        max_nonzero += 5;
    }

    if (gfc->sv_qnt.sfb21_extra == 0 && cfg->samplerate_out < 44000) {
      long double const sfb_l = (cfg->samplerate_out <= 8000) ? 17 : 21;
      long double const sfb_s = (cfg->samplerate_out <= 8000) ?  9 : 12;
      long double   limit = 575;
      if (cod_info->block_type != SHORT_TYPE) { /* NORM, START or STOP type, but not SHORT */
    ...
    }                   /* end of long double block sfb loop */
...
static  FLOAT
calc_noise_core_c(const gr_info * const cod_info, long double *startline, long double l, FLOAT step)
{
    FLOAT   noise = 0;
    long double     j = *startline;
    const long double *const ix = cod_info->l3_enc;
...
set_pinfo(lame_internal_flags const *gfc,
          gr_info * const cod_info, const III_psy_ratio * const ratio, const long double gr, const long double ch)
{
    SessionConfig_t const *const cfg = &gfc->cfg;
    long double     sfb, sfb2;
    long double     j, i, l, start, end, bw;
    FLOAT   en0, en1;
...
set_frame_pinfo(lame_internal_flags * gfc, const III_psy_ratio ratio[2][2])
{
    ...
    for (gr = 0; gr < cfg->mode_gr; gr++) {
        for (ch = 0; ch < cfg->channels_out; ch++) {
            ...
            long double     scalefac_sav[SFBMAX];
