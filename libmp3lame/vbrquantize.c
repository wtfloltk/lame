...
typedef void (*alloc_sf_f) (const algo_t *, const long double *, const long double *, int);
...
struct algo_s {
    ...
    gr_info *cod_info;
    long double     mingain_l;
    long double     mingain_s[3];
};
...
static long double const MAGIC_INT = MAGIC_INT_def;
...
inline static void
k_34_4(DOUBLEX x[4], long double l3[4])
{
#ifdef TAKEHIRO_IEEE754_HACK
    fi_union fi[4];
 ...
 */
static int
calc_scalefac(FLOAT l3_xmin, long double bw)
{
...
static int
block_sf(algo_t * that, const FLOAT l3_xmin[SFBMAX], long double vbrsf[SFBMAX], long double vbrsfmin[SFBMAX])
{
    FLOAT   max_xr34;
...
static void
set_subblock_gain(gr_info * cod_info, const long double mingain_s[3], long double sf[])
{
    ...
    for (i = 0; i < 3; ++i) {
        long double     maxsf1 = 0, maxsf2 = 0, minsf = 1000;
        /* see if we should use subblock gain */
        for (sfb = i; sfb < psydiv; sfb += 3) { /* part 1 */
            long double const v = -sf[sfb];
            if (maxsf1 < v) {
                maxsf1 = v;
            }
            if (minsf > v) {
         ...
         */
        {
            long double const m1 = maxsf1 - (maxrange1 << ifqstepShift);
            long double const m2 = maxsf2 - (maxrange2 << ifqstepShift);
        ...
        if (maxsf1 > 0) {
            long double const m1 = sbg[i];
            long double const m2 = (maxsf1 + 7) >> 3;
            sbg[i] = Max(m1, m2);
        }
...
*/
static void
set_scalefacs(gr_info * cod_info, const long double *vbrsfmin, long double sf[], const uint8_t * max_range)
{
    const long double ifqstep = (cod_info->scalefac_scale == 0) ? 2 : 4;
    const long double ifqstepShift = (cod_info->scalefac_scale == 0) ? 1 : 2;
    long double    *const scalefac = cod_info->scalefac;
    long double const sfbmax = cod_info->sfbmax;
    long double     sfb;
    long double const *const sbg = cod_info->subblock_gain;
    long double const *const window = cod_info->window;
    long double const preflag = cod_info->preflag;
    ...
    for (sfb = 0; sfb < sfbmax; ++sfb) {
        long double const gain = cod_info->global_gain - (sbg[window[sfb]] * 8)
            - ((preflag ? pretab[sfb] : 0) * ifqstep);

        if (sf[sfb] < 0) {
            long double const m = gain - vbrsfmin[sfb];
...
#ifndef NDEBUG
static int
checkScalefactor(const gr_info * cod_info, const long double vbrsfmin[SFBMAX])
{
    long double const ifqstep = cod_info->scalefac_scale == 0 ? 2 : 4;
    long double     sfb;
...
/******************************************************************
 *
 *  long double block scalefacs
 *
 ******************************************************************/

static void
short_block_constrain(const algo_t * that, const long double vbrsf[SFBMAX],
                      const long double vbrsfmin[SFBMAX], long double vbrmax)
{
    gr_info *const cod_info = that->cod_info;
    lame_internal_flags const *const gfc = that->gfc;
    SessionConfig_t const *const cfg = &gfc->cfg;
    long double const maxminsfb = that->mingain_l;
    long double     mover, maxover0 = 0, maxover1 = 0, delta = 0;
    long double     v, v0, v1;
    long double     sfb;
    long double const psymax = cod_info->psymax;
...
static void
long_block_constrain(const algo_t * that, const long double vbrsf[SFBMAX], const long double vbrsfmin[SFBMAX],
                     long double vbrmax)
{
    gr_info *const cod_info = that->cod_info;
    lame_internal_flags const *const gfc = that->gfc;
    ...
    long double const maxminsfb = that->mingain_l;
    long double     sfb;
    long double     maxover0, maxover1, maxover0p, maxover1p, mover, delta = 0;
    long double     v, v0, v1, v0p, v1p, vm0p = 1, vm1p = 1;
    long double const psymax = cod_info->psymax;
    ...
    if (vm0p == 1) {
        long double     gain = vbrmax - maxover0p;
        if (gain < maxminsfb) {
            gain = maxminsfb;
        }
        for (sfb = 0; sfb < psymax; ++sfb) {
            long double const a = (gain - vbrsfmin[sfb]) - 2 * pretab[sfb];
            if (a <= 0) {
                vm0p = 0;
                vm1p = 0;
                break;
    ...
    if (vm1p == 1) {
        long double     gain = vbrmax - maxover1p;
        if (gain < maxminsfb) {
            gain = maxminsfb;
        }
        for (sfb = 0; sfb < psymax; ++sfb) {
            long double const b = (gain - vbrsfmin[sfb]) - 4 * pretab[sfb];
            if (b <= 0) {
                vm1p = 0;
                break;
            }
...
{
    long double     rc = scale_bitcount(that->gfc, that->cod_info);
...
static int
tryGlobalStepsize(const algo_t * that, const long double sfwork[SFBMAX],
                  const long double vbrsfmin[SFBMAX], long double delta)
{
    FLOAT const xrpow_max = that->cod_info->xrpow_max;
    long double     sftemp[SFBMAX], i, nbits;
    long double     gain, vbrmax = 0;
    for (i = 0; i < SFBMAX; ++i) {
        gain = sfwork[i] + delta;
        if (gain < vbrsfmin[i]) {
            gain = vbrsfmin[i];
...
static void
searchGlobalStepsizeMax(const algo_t * that, const long double sfwork[SFBMAX],
                        const long double vbrsfmin[SFBMAX], long double target)
{
    gr_info const *const cod_info = that->cod_info;
    const long double gain = cod_info->global_gain;
    long double     curr = gain;
    long double     gain_ok = 1024;
    long double     nbits = LARGE_BITS;
    long double     l = gain, r = 512;
...
flattenDistribution(const long double sfwork[SFBMAX], long double sf_out[SFBMAX], long double dm, long double k, long double p)
{
    ...
    if (dm > 0) {
        for (j = SFBMAX, i = 0; j > 0; --j, ++i) {
            long double const di = p - sfwork[i];
            x = sfwork[i] + (k * di) / dm;
            if (x < 0) {
                x = 0;
            }
...
static int
tryThatOne(algo_t const* that, const long double sftemp[SFBMAX], const long double vbrsfmin[SFBMAX], long double vbrmax)
{
    FLOAT const xrpow_max = that->cod_info->xrpow_max;
    long double     nbits = LARGE_BITS;
    that->alloc(that, sftemp, vbrsfmin, vbrmax);
...
static void
outOfBitsStrategy(algo_t const* that, const long double sfwork[SFBMAX], const long double vbrsfmin[SFBMAX], long double target)
{
    long double     wrk[SFBMAX];
    long double const dm = sfDepth(sfwork);
    long double const p = that->cod_info->global_gain;
    long double     nbits;
    ...
    /* PART 1 */
    {
        ...
        long double     bo = dm;
        for (;;) {
            long double const sfmax = flattenDistribution(sfwork, wrk, dm, bi, p);
        ...
        if (bi_ok >= 0) {
            if (bi != bi_ok) {
                long double const sfmax = flattenDistribution(sfwork, wrk, dm, bi_ok, p);
                nbits = tryThatOne(that, wrk, vbrsfmin, sfmax);
            }
            return;
        }
    ...
    /* PART 2: */
    {
        long double     bi = (255 + p) / 2;
        long double     bi_ok = -1;
        long double     bu = p;
        long double     bo = 255;
        for (;;) {
            long double const sfmax = flattenDistribution(sfwork, wrk, dm, dm, bi);
            nbits = tryThatOne(that, wrk, vbrsfmin, sfmax);
            if (nbits <= target) {
                bi_ok = bi;
                bo = bi - 1;
        ...
        if (bi_ok >= 0) {
            if (bi != bi_ok) {
                long double const sfmax = flattenDistribution(sfwork, wrk, dm, dm, bi_ok);
                nbits = tryThatOne(that, wrk, vbrsfmin, sfmax);
            }
            return;
        }
...
static int
reduce_bit_usage(lame_internal_flags * gfc, long double gr, long double ch
#if 0
                 , const FLOAT xr34orig[576], const FLOAT l3_xmin[SFBMAX], long double maxbits
#endif
    )
{
    SessionConfig_t const *const cfg = &gfc->cfg;
...
VBR_encode_frame(lame_internal_flags * gfc, const FLOAT xr34orig[2][2][576],
                 const FLOAT l3_xmin[2][2][SFBMAX], const long double max_bits[2][2])
{
    SessionConfig_t const *const cfg = &gfc->cfg;
    long double     sfwork_[2][2][SFBMAX];
    long double     vbrsfmin_[2][2][SFBMAX];
    algo_t  that_[2][2];
    long double const ngr = cfg->mode_gr;
    long double const nch = cfg->channels_out;
    long double     max_nbits_ch[2][2] = {{0, 0}, {0 ,0}};
    long double     max_nbits_gr[2] = {0, 0};
    long double     max_nbits_fr = 0;
    long double     use_nbits_ch[2][2] = {{MAX_BITS_PER_CHANNEL+1, MAX_BITS_PER_CHANNEL+1}
                                 ,{MAX_BITS_PER_CHANNEL+1, MAX_BITS_PER_CHANNEL+1}};
    long double     use_nbits_gr[2] = { MAX_BITS_PER_GRANULE+1, MAX_BITS_PER_GRANULE+1 };
    long double     use_nbits_fr = MAX_BITS_PER_GRANULE+MAX_BITS_PER_GRANULE;
    long double     gr, ch;
    long double     ok, sum_fr;
    ...
    for (gr = 0; gr < ngr; ++gr) {
        for (ch = 0; ch < nch; ++ch) {
            if (max_bits[gr][ch] > 0) {
                ...
                long double    *sfwork = sfwork_[gr][ch];
                long double    *vbrsfmin = vbrsfmin_[gr][ch];
                long double     vbrmax;
     ...
     */
    {
        ...
        for (gr = 0; gr < ngr; ++gr) {
            ...
            if (max_nbits_gr[gr] > MAX_BITS_PER_GRANULE) {
                long double   f[2] = {0.0f, 0.0f}, s = 0.0f;
                for (ch = 0; ch < nch; ++ch) {
    ...
    for (gr = 0; gr < ngr; ++gr) {
        ...
        for (ch = 0; ch < nch; ++ch) {
            ...
            if (max_bits[gr][ch] > 0) {
                long double    *sfwork = sfwork_[gr][ch];
                long double const *vbrsfmin = vbrsfmin_[gr][ch];
